    ...
    <>
      ...
      <motion.aside
        ...
        className={`
          ...
          left-0 h-screen lg:h-fit lg:max-h-[calc(100vh-6rem)]
          w-80 bg-surface-container-low lg:rounded-xl p-6 
          overflow-hidden z-50 lg:z-40 border-r lg:border-none border-outline-variant/20
          ...
          {social.map((link) => (
            link.name !== "Email" && (
              <Link
                ...
                href={link.href}
                target="_blank"
                className="text-on-surface-variant hover:text-primary transition-colors tooltip relative group flex items-center justify-center w-8 h-8"
              >
                {link.icon === "github" ? (
